...
def observations(S, length=None, downbeat=None, nextDownbeat=None, score=0, level=0, parent=None, performance=False, verbose=False):
  ...
  if verbose > 2:
    print 'downbeat: {0}, nextDownbeat: {1}, downbeats {2}, spans: {3}'.format(downbeat, nextDownbeat, downbeats, spans)
    S.view(showOnsets=True, showFeatures=True, scale=False)
  ...
  if len(spans) > 0 and lscore >= score:
    ...
    if downbeat == None or nextDownbeat == None:
      return []
    #  downbeat = beats[0]
