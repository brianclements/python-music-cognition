...
def observations(S, downbeat=None, nextDownbeat=None, level=0, parent=None, performance=False, verbose=False):
  ...
  for child, beat, onset, i in zip(S.children, beats, onsets, range(0, division)):
    #if beat != None and i != 0:
    if onset != None and beat != None and i != 0:
      obs.append(features(downbeat, nextDownbeat, onset, i, division, level))
